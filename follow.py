...
__________________________________________________________________________________________________________________________
 ...
 [\x1b[1;96m+%s] Author   : Mirwais Danishyar

 [\x1b[1;96m+%s] Github   : github.com/mirwaiskhulmi

 [\x1b[1;95m+%s] Facebook : Mirwais Danishyar

 [\x1b[1;94m+%s] suport kartu : [ AWM ]
...
class menu:
	...
	def main(self):
		...
		print(' %s[%s0%s] crack from daftar teman'%(M,O,M))

		print(' %s[%s1%s] crack from AWM publik'%(M,O,M))

		print(' %s[%s2%s] crack from AWM massal [error]'%(M,O,M))

		print(' %s[%s3%s] crack from post'%(M,O,M))

		print(' %s[%s4%s] crack from likes post'%(M,O,M))

		print(' %s[%s5%s] crack from followers'%(M,O,M))

		print(' %s[%s6%s] cek opsi AWM chekpoint'%(N,O,N))
...
class crack:
				...
				if pw in ['']:
					...
					def manual(xnxx=None):
						...
						if mani in ['']:
							...
							print('  [%s*%s] AWM Ok  save file on   : Ok.txt'%(O,N))

							print('  [%s*%s] AWM CP  save file on   : Cp.txt\n'%(O,N))
							...
							print(' %s [%s*%s] AWM OK  save file on   : Ok.txt'%(N,O,N))

							print(' %s [%s*%s] AWM CP   save file on   : Cp.txt\n'%(N,O,N))
	...
	def free(self):
		...
		print(' %s [%s*%s] AWM  CP save file on  : hasil/okeh.txt'%(N,O,N))

		print(' %s [%s*%s] AWM  CP save file on : hasil/cepeh.txt\n'%(N,O,N))
	...
	def basic(self):
		...
		print(' %s [%s*%s] AWM OK akan di simpan di file : Ok.txt'%(N,O,N))

		print(' %s [%s*%s] AWM CP akan di simpan di file : Cp.txt'%(N,O,N))
